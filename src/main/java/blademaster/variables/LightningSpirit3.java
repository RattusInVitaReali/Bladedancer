...
public class LightningSpirit3 extends DynamicVariable {   // Custom Dynamic Variables are what you do if you need your card text to display a cool, changing number that the base game doesn't provide.
    ...
    public int value(AbstractCard card) {
        if (CardCrawlGame.isInARun()) {
            if (AbstractDungeon.player.hasPower(LightningCharge.POWER_ID)) {
                return 3 * AbstractDungeon.player.getPower(LightningCharge.POWER_ID).amount;
            } else {
                return 0;
            }
        } else {
