...
public class IceSpirit extends DynamicVariable {   // Custom Dynamic Variables are what you do if you need your card text to display a cool, changing number that the base game doesn't provide.
    ...
    public int value(AbstractCard card) {
        if (CardCrawlGame.isInARun()) {
            if (AbstractDungeon.player.hasPower(IceCharge.POWER_ID)) {
                return AbstractDungeon.player.getPower(IceCharge.POWER_ID).amount;
            } else {
                return 0;
            }
        } else {
