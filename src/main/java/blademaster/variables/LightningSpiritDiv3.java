...
public class LightningSpiritDiv3 extends DynamicVariable {   // Custom Dynamic Variables are what you do if you need your card text to display a cool, changing number that the base game doesn't provide.
    ...
    public int value(AbstractCard card) {
        if (CardCrawlGame.isInARun()) {
            if (AbstractDungeon.player.hasPower(LightningCharge.POWER_ID)) {
                return AbstractDungeon.player.getPower(LightningCharge.POWER_ID).amount / 3;
            } else {
                return 0;
            }
        } else {
