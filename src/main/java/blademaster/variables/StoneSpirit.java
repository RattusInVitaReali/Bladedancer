...
public class StoneSpirit extends DynamicVariable {   // Custom Dynamic Variables are what you do if you need your card text to display a cool, changing number that the base game doesn't provide.
    ...
    public int value(AbstractCard card) {
        if (CardCrawlGame.isInARun()) {
            if (AbstractDungeon.player.hasPower(StoneCharge.POWER_ID)) {
                return AbstractDungeon.player.getPower(StoneCharge.POWER_ID).amount;
            } else {
                return 0;
            }
        } else {
