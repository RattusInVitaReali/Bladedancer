...
public class Blindside extends AbstractStanceCard {
    ...
    public void update() {
        ...
        if (WindArtS) {
            if (! WindArt) {
                ...
                this.rawDescription = DESCRIPTION + EXTENDED_DESCRIPTION[0] + EXTENDED_DESCRIPTION[2];
            ...
            if (! LightningArt) {
                ...
                this.rawDescription = DESCRIPTION + EXTENDED_DESCRIPTION[1] + EXTENDED_DESCRIPTION[2];
                initializeDescription();
                WindArt = false;
                LightningArt = true;
                BaseArt = false;
    ...
    public void hover() {
        if (this.cardToPreview1 == null && ! this.bullshit) {
            this.cardToPreview1 = new Blindside();
            this.cardToPreview2 = new Blindside();
            if (this.upgraded || SingleCardViewPopup.isViewingUpgrade) {
                this.cardToPreview1.upgrade();
                this.cardToPreview2.upgrade();
            }
            ((Blindside) this.cardToPreview1).WindArtS = true;
            this.cardToPreview1.update();
            ((Blindside) this.cardToPreview2).LightningArtS = true;
